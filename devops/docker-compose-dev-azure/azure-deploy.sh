...
resourcename="paymentmanagement"  # lower case, only
resourcegroup="paymentmanagement_rg"
...
# git clone https://github.com/apilogicserver/paymentmanagement.git
...
az group create --name paymentmanagement_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group paymentmanagement_rg --sku S1 --is-linux
az container create --resource-group paymentmanagement_rg --name paymentmanagement --image apilogicserver/paymentmanagement:latest --dns-name-label paymentmanagement --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//paymentmanagement.westus.azurecontainer.io:5656'
